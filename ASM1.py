...
if date_col and study_col:
    df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
    trend = df.groupby(date_col)[study_col].mean()

    plt.figure(figsize=(10,5))
    plt.plot(trend.index, trend.values)
    plt.title("Xu huong hoc theo thoi gian")
    plt.xlabel("Thoi gian")
    plt.ylabel("Thoi gian hoc trung binh")
    plt.xticks(rotation=30)
    plt.savefig("charts/xu_huong_hoc_theo_thoi_gian.png")
    plt.show()
